...
func TestInvalidKeyStore(t *testing.T) {
	...
	ks, err := NewFileBasedKeyStore([]byte("123456"), filepath.Join(tempDir, "bccspks"), false)
	assert.NoError(t, err)

	err = ks.StoreKey(nil)
	assert.EqualError(t, err, "invalid key, it must be different from nil")

	err = ks.StoreKey(&ecdsaPrivateKey{privateKey: nil})
	assert.EqualError(t, err, "failed storing ECDSA private key [invalid ecdsa private key, it must be different from nil]")

	err = ks.StoreKey(&ecdsaPublicKey{publicKey: nil})
	assert.EqualError(t, err, "failed storing ECDSA public key [invalid ecdsa public key, it must be different from nil]")

	err = ks.StoreKey(&aesKey{key: nil, exportable: false})
	assert.EqualError(t, err, "failed storing AES key [invalid aes key, it must be different from nil]")

	err = ks.StoreKey(&aesKey{key: nil, exportable: true})
	assert.EqualError(t, err, "failed storing AES key [invalid aes key, it must be different from nil]")
...
func TestBigKeyFile(t *testing.T) {
	...
	ski := cspKey.SKI()
	rawKey, err := ecdsaPrivateKeyToPEM(privKey, nil)
	require.NoError(t, err)
