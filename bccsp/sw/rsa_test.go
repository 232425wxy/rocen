...
func TestRSAPublicKey(t *testing.T) {
	...
	k.publicKey = &lowLevelKey.PublicKey
	ski = k.SKI()
	raw, err := asn1.Marshal(rsaPublicKeyASN{N: k.publicKey.N, E: k.publicKey.E})
	...
	bytes, err := k.Bytes()
	require.NoError(t, err)
	bytes2, err := x509.MarshalPKIXPublicKey(k.publicKey)
	require.Equal(t, bytes2, bytes, "bytes are not computed in the right way.")

	_, err = (&rsaPublicKey{}).Bytes()
	require.EqualError(t, err, "rsa public key is nil")
}
