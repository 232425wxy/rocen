...
func (e *Encryptor) Encrypt(key bccsp.Key, plaintext []byte, opts bccsp.EncrypterOpts) ([]byte, error) {
	if !reflect.DeepEqual(e.KeyArg, key) {
...
func (d *Decryptor) Decrypt(key bccsp.Key, ciphertext []byte, opts bccsp.DecrypterOpts) (plaintext []byte, err error) {
	panic("implement me")
}
...
func (s *Signer) Sign(key bccsp.Key, digest []byte, opts bccsp.SignerOpts) ([]byte, error) {
	if !reflect.DeepEqual(key, s.KeyArg) {
		return nil, errors.New("invalid key")
	}
